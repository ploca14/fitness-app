...
function CreateProgram() {
  ...
  const { state: prefetchedClient } = useLocation();
  const { isLoading, isError, data: client, error } = useQuery(['user', id], () => apiFetch(`/Users/${id}`), { initialData: prefetchedClient })
  const history = useHistory();
  const queryClient = useQueryClient();

  const mutation = useMutation(newProgram => {
    return apiFetch('/WorkoutPrograms', {
      method: 'POST',
      body: {
        ...newProgram,
  ...
  if (isLoading) {
    return <Page pageName="Loading client..."></Page>;
  }
